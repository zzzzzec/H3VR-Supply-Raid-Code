    ...
    public class SR_Manager : MonoBehaviour
    {
        ...
        */
        ...
        public void LaunchGame()
        {
            ...
            // Check if the faction file provides a point override
            if (faction.pointsLevelOverride != null && faction.pointsLevelOverride.Length > 0)
            {
                // If it does, replace the character's point levels with the faction's
                character.pointsLevel = faction.pointsLevelOverride;
                Debug.Log("Supply Raid: Faction (" + faction.name + ") has overridden character points.");
            }
            ...
            SetupSupplyPoints();
            Debug_ComparePathfindingStrategies();
        ...
        /// <summary>
        /// 使用“最近邻”启发式算法为给定的据点列表生成一条近似最短的遍历路径（解决旅行商问题）。
        /// </summary>
        /// <param name="pointsToVisit">需要排序的原始据点列表。</param>
        /// <returns>一个按照近似最短路径排序的新的据点列表。</returns>
        private List<SR_SupplyPoint> GenerateShortestPath_TSP(List<SR_SupplyPoint> pointsToVisit, SR_SupplyPoint currentPoint)
        {
            if (pointsToVisit == null || pointsToVisit.Count <= 1)
            {
                return new List<SR_SupplyPoint>(pointsToVisit);
            }
            List<SR_SupplyPoint> path = new List<SR_SupplyPoint>();
            List<SR_SupplyPoint> remainingPoints = new List<SR_SupplyPoint>(pointsToVisit);
            path.Add(currentPoint);
            remainingPoints.Remove(currentPoint);
            while (remainingPoints.Count > 0)
            {
                SR_SupplyPoint nearestPoint = null;
                float minDistanceSqr = float.MaxValue;
                foreach (SR_SupplyPoint potentialNextPoint in remainingPoints)
                {
                    float distanceSqr = (potentialNextPoint.captureZone.position - currentPoint.captureZone.position).sqrMagnitude;
                    if (distanceSqr < minDistanceSqr)
                    {
                        minDistanceSqr = distanceSqr;
                        nearestPoint = potentialNextPoint;
                    }
                }
        ...
        /// <summary>
        /// 使用“最近邻”启发式算法生成一条近似最短的遍历路径（解决旅行商问题），
        /// 并增加了“连续节点距离必须大于50米”的约束条件。
        /// 备用方案经过优化，会选择相对最远的点以保证游戏性。
        /// </summary>
        /// <param name="pointsToVisit">需要排序的原始据点列表。</param>
        /// <param name="currentPoint">路径的起始点。</param>
        /// <returns>一个按照新规则排序的、新的据点列表。</returns>
        private List<SR_SupplyPoint> GenerateShortestPath_TSP_WithDistanceConstraint(List<SR_SupplyPoint> pointsToVisit, SR_SupplyPoint currentPoint)
        {
            if (pointsToVisit == null || pointsToVisit.Count <= 1)
            {
                return new List<SR_SupplyPoint>(pointsToVisit);
            ...
            while (remainingPoints.Count > 0)
            {
                SR_SupplyPoint finalNextPoint = null;
                SR_SupplyPoint nearestFarPoint = null;
                float minFarDistanceSqr = float.MaxValue;
                foreach (SR_SupplyPoint potentialNextPoint in remainingPoints)
                {
                    float distanceSqr = (potentialNextPoint.captureZone.position - currentPoint.captureZone.position).sqrMagnitude;

                    if (distanceSqr > MinDistanceSqr && distanceSqr < minFarDistanceSqr)
                    {
                        minFarDistanceSqr = distanceSqr;
                        nearestFarPoint = potentialNextPoint;
                    }
                }

                if (nearestFarPoint != null)
                {
                    finalNextPoint = nearestFarPoint;
                }
                else
                {
                    SR_SupplyPoint farthestClosePoint = null; 
                    float maxCloseDistanceSqr = -1f;
                    foreach (SR_SupplyPoint potentialNextPoint in remainingPoints)
                    {
                        float distanceSqr = (potentialNextPoint.captureZone.position - currentPoint.captureZone.position).sqrMagnitude;
                        if (distanceSqr > maxCloseDistanceSqr)
                        {
                            maxCloseDistanceSqr = distanceSqr;
                            farthestClosePoint = potentialNextPoint;
                        }
                    }
                    finalNextPoint = farthestClosePoint;
                    if (farthestClosePoint != null)
                    {
                        float actualDistance = Mathf.Sqrt(maxCloseDistanceSqr);
                        Debug.Log("[TSP Fallback Activated] No points > 50m found. Selected the farthest available point: '"
                                  + farthestClosePoint.name
                                  + "' at a distance of "
                                  + actualDistance.ToString("F2") + "m.");
                    }
                }
                if (finalNextPoint != null)
                {
                    currentPoint = finalNextPoint;
                    path.Add(currentPoint);
                    remainingPoints.Remove(currentPoint);
                }
                else
                {
                    Debug.LogError("Supply Raid: TSP algorithm failed to find any next point. Breaking loop.");
                    break;
                }
            }
            return path;
        ...
        /// <summary>
        /// 一个调试函数，用于在Unity控制台清晰地打印出最终生成的supplyOrder路径及其距离。
        /// </summary>
        private void Debug_PrintFinalPath()
        {
            Debug.Log("=============== FINAL SUPPLY ORDER PATH ===============");
            if (supplyOrder == null || supplyOrder.Count == 0)
            {
                Debug.Log("SupplyOrder is empty. No path to print.");
                Debug.Log("=====================================================");
                return;
            }
            ...
            {
                int currentPointIndex = supplyOrder[i];
                SR_SupplyPoint currentPoint = supplyPoints[currentPointIndex];

                int nextPointIndexInList = (i + 1) % supplyOrder.Count;
                int nextPointIndex = supplyOrder[nextPointIndexInList];
                SR_SupplyPoint nextPoint = supplyPoints[nextPointIndex];

                float distance = Vector3.Distance(currentPoint.captureZone.position, nextPoint.captureZone.position);
                totalDistance += distance;
                string pathSegment = string.Format("[Step {0}]: {1} --> {2} | Distance: {3:F2}m",
                    i,
                    currentPoint.name,
                    nextPoint.name,
                    distance);
            ...
            Debug.Log("-----------------------------------------------------");
            Debug.Log(string.Format("TOTAL PATH DISTANCE: {0:F2}m", totalDistance));
            Debug.Log("=====================================================");
        }
        /// <summary>
        /// 【调试专用】运行所有路径规划策略，并在控制台打印出总距离的对比总结。
        /// </summary>
        public void Debug_ComparePathfindingStrategies()
        {
            // --- 准备工作 ---
            // 记录下当前真实的 captureOrder，以便测试后恢复
            int originalCaptureOrder = profile.captureOrder;
            Debug.Log("=============== PATHFINDING STRATEGY COMPARISON ===============");
            // --- 策略1: RANDOM ORDER (有序随机) ---
            profile.captureOrder = 0;
            supplyOrder.Clear();
            SetupSupplyPoints(); // 运行该策略的路径生成逻辑
            float distance_RandomOrder = CalculatePathTotalDistance(); // 计算总距离
            Debug.Log(string.Format("Strategy: [RANDOM ORDER] | Total Distance: {0:F2}m", distance_RandomOrder));
            // --- 策略2: ORDERED (完全固定) ---
            profile.captureOrder = 2;
            supplyOrder.Clear();
            SetupSupplyPoints();
            float distance_Ordered = CalculatePathTotalDistance();
            Debug.Log(string.Format("Strategy: [ORDERED]      | Total Distance: {0:F2}m", distance_Ordered));
            // --- 策略4: SHORTEST PATH (最近邻TSP) ---
            profile.captureOrder = 3;
            supplyOrder.Clear();
            SetupSupplyPoints();
            float distance_TSP = CalculatePathTotalDistance();
            Debug.Log(string.Format("Strategy: [TSP ]        | Total Distance: {0:F2}m", distance_TSP));

            Debug.Log("============================================================");
            // 恢复游戏原始的 captureOrder 设置，避免影响正常游戏
            profile.captureOrder = originalCaptureOrder;
            // 重新运行一次原始设置的路径生成，确保游戏状态正确
            supplyOrder.Clear();
            SetupSupplyPoints();
            Debug.Log("Original capture order has been restored.");
        }

        /// <summary>
        /// 【辅助函数】只计算并返回当前 supplyOrder 的总距离，不打印每一步。
        /// </summary>
        private float CalculatePathTotalDistance()
        {
            if (supplyOrder == null || supplyOrder.Count == 0) return 0f;

            float totalDistance = 0f;
            for (int i = 0; i < supplyOrder.Count; i++)
            {
                SR_SupplyPoint currentPoint = supplyPoints[supplyOrder[i]];
                int nextPointIndexInList = (i + 1) % supplyOrder.Count;
                SR_SupplyPoint nextPoint = supplyPoints[supplyOrder[nextPointIndexInList]];
                totalDistance += Vector3.Distance(currentPoint.captureZone.position, nextPoint.captureZone.position);
            }
        ...
        void SetupSupplyPoints()
        {
            ...
            else if (profile.captureOrder == 3) // SHORTEST PATH (TSP)
            {
                List<SR_SupplyPoint> rootPoints = new List<SR_SupplyPoint>();
                for (int i = 0; i < supplyPoints.Count; i++)
                {
                    if (supplyPoints[i].previousSupplyPoint == null)
                    {
                        rootPoints.Add(supplyPoints[i]);
                    }
                }
                // List<SR_SupplyPoint> orderedRoots = GenerateShortestPath_TSP(rootPoints, supplyPoints[0]);
                SR_SupplyPoint startPoint;
                if (rootPoints.Count > 0)
                {
                    int randomIndex = Random.Range(0, rootPoints.Count);
                    startPoint = rootPoints[randomIndex];
                }
                else
                {
                    Debug.LogWarning("Supply Raid: TSP 路径规划未找到任何根节点, 将默认使用 supplyPoints[0] 作为起点。");
                    startPoint = supplyPoints[0];
                }
                // List<SR_SupplyPoint> orderedRoots = GenerateShortestPath_TSP(rootPoints, startPoint);
                // 3. 修改下面这一行，使用新的 startPoint 变量
                List<SR_SupplyPoint> orderedRoots = GenerateShortestPath_TSP_WithDistanceConstraint(rootPoints, startPoint);
                // List<SR_SupplyPoint> orderedRoots = GenerateShortestPath_TSP_WithDistanceConstraint(rootPoints, supplyPoints[0]);
                // 3. 将排序好的、近似最短的路径，展开成最终的 supplyOrder 列表
                for (int x = 0; x < orderedRoots.Count; x++)
                {
                    supplyOrder.Add(orderedRoots[x].index);

                    // 如果这个起点有后续路径，也一并添加进来
                    if (orderedRoots[x].nextSupplyPoint)
                    {
                        SR_SupplyPoint scannedSP = orderedRoots[x].nextSupplyPoint;
                        while (scannedSP != null)
                        {
                            if (scannedSP)
                                supplyOrder.Add(scannedSP.index);
                ...
                // 后续的 FORCED SPAWN SUPPLY 和 FIRST ATTACK POINT 逻辑可以复用模式0/模式2的
                // (这里为了完整性，我直接复制过来了)
                for (int i = 0; i < supplyOrder.Count; i++)
                {
                    if (supplyPoints[supplyOrder[i]].forceFirstSpawn)
