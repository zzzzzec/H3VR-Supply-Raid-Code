    ...
    public class SR_MagazineDuplicator : MonoBehaviour
    {
        ...
        private void SetupCosts()
        {
            ...
            if (m_detectedMag != null)
            {
                ...
                for (int i = 0; i < mags.Count; i++)
                {
                    ...
                    {
                        // 太大的不要
                        if (mags[i].MagazineCapacity > m_detectedMag.m_capacity && mags[i].MagazineCapacity < num && mags[i].MagazineCapacity <= 99)
                        {
        ...
        public void Button_Upgrade()
        {
            ...
            if (magazineUpgrade != null)
            {
                ...
                {
                    if (m_detectedMag.LoadedRounds[i] != null)
                    {
                        oldRoundClasses.Add(m_detectedMag.LoadedRounds[i].LR_Class);
                    }
                }
                SR_Manager.SpendPoints(costUpgrade);
                Destroy(m_detectedMag.GameObject);
                // GameObject spawnedMagazine = Instantiate(magazineUpgrade.GetGameObject(), Spawnpoint_Mag.position, Spawnpoint_Mag.rotation);
                GameObject spawnedMagazineGO = Instantiate(magazineUpgrade.GetGameObject(), Spawnpoint_Mag.position, Spawnpoint_Mag.rotation);
                FVRFireArmMagazine spawnedMagazine = spawnedMagazineGO.GetComponent<FVRFireArmMagazine>();
                if (spawnedMagazine != null)
                {
                    spawnedMagazine.ReloadMagWithList(oldRoundClasses);
                }
                SR_Manager.PlayConfirmSFX();
            }
            else
